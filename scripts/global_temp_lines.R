...
#load data
download.file(url = "https://data.giss.nasa.gov/gistemp/tabledata_v4/GLB.Ts+dSST.csv", destfile = "data/climate/GLB.Ts+dSST.csv", method = "auto")

t_diff <- read_csv("data/climate/GLB.Ts+dSST.csv", skip = 1, na = "***") %>%
...
t_data %>%
  ...
  annotate("text", x = 3.2, 
           y = -0.9, 
           label = "The reference 0ºC level corresponds to deviations from the 1951-1980 means", 
           color = "white",
           size = 2)
