...
e13a_bet_n_01_average_below <- e13a_bet_n_01_average %>%
  select(matches("^E13ABetn01r"), matches("^E13ABetn01s01"))

# Select columns starting for above the ground and the base
e13a_bet_n_01_average_above <- e13a_bet_n_01_average %>%
  select(matches("^E13ABetn01s"), matches("^E13ABetn01t"))

#correlation test E13A01 below
E13A01below_cor_rest <- cor(e13a_bet_n_01_average_below$E13ABetn01s01, e13a_bet_n_01_average_below, 
                              use = "pairwise.complete.obs", method = "spearman")

#Check correlations
mean(E13A01below_cor_rest)

# Convert correlation matrix to a data frame
E13A_01_below_cor_df <- as.data.frame(E13A01below_cor_rest)

# Add a column for the variable names
E13A_01_below_cor_df$variable <- rownames(E13A_01_below_cor_df)

# Reshape the data frame to long format for plotting
E13A_01_below_cor_df_long <- pivot_longer(E13A_01_below_cor_df, -variable, names_to = "section", values_to = "e13a01_correlation")
...
base_e13_indv <- rbind(
  data.frame(x="E13A_01", category = E13A_01_cor_df_long$category, 
             correlation = E13A_01_cor_df_long$e13a01_correlation, category = E13A_01_cor_df_long$category),
  data.frame(x="E13A_02", category = E13A_01_cor_df_long$category, 
             correlation = E13A_02_cor_df_long$e13a02_correlation, category = E13A_02_cor_df_long$category),
  data.frame(x="E13A_03", category = E13A_01_cor_df_long$category,
             correlation =E13A_03_cor_df_long$e13a03_correlation, category = E13A_03_cor_df_long$category)
  )
...
base_e13c_indv <- rbind(
  data.frame(x="E13C_01", category = E13C_01_cor_df_long$category, 
             correlation = E13C_01_cor_df_long$e13c01_correlation, category = E13C_01_cor_df_long$category),
  data.frame(x="E13C_03", category = E13C_03_cor_df_long$category, 
             correlation = E13C_03_cor_df_long$e13c03_correlation, category = E13C_03_cor_df_long$category),
  data.frame(x="E13C_07", category = E13C_07_cor_df_long$category,
             correlation = E13C_07_cor_df_long$e13c07_correlation, category = E13C_07_cor_df_long$category)
  )
...
ggplot(base_e13v, aes(x=x, y=correlation, color = category, shape = shape)) +
  geom_boxplot(width=.2, fill = "white", color = "black") +
  geom_jitter(width=0.1) +
  scale_color_manual(values = color_palette) +
